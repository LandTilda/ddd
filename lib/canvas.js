var $shapes = [];
var $selector = '.canvas-header .shape';
...
var Nodes = {
  ...
  disableExplosion: 'off',
  rotateTimer: parseInt('5000'),
  ...
  init: function(canvasID) {
    ...
    this.canvas.width = ($(canvasID).closest('.canvas-header').length > 0 ) ? window.innerWidth : $(canvasID).closest('.canvas-bg').width();
    this.canvas.height = ($(canvasID).closest('.canvas-header').length > 0 ) ? window.innerHeight : $(canvasID).closest('.canvas-bg').height();
    ...
    */
    ...
    //default autorotate
    Nodes.rotateTimer = 5500;

    if($(canvasID).parents('.canvas-header').length > 0) {
	    ...
	    //});
    	 ...
    	 $(canvasID).parents('.canvas-bg').on('mousemove',function(e){
	    	Nodes.mouse.x = e.clientX - $(this).offset().left;
	    	Nodes.mouse.y = e.clientY - $(this).offset().top + $(window).scrollTop();
	    });
	    $(canvasID).parents('.canvas-bg').on('mouseout',function(){
	    	Nodes.mouse.x = 1000;
	  		Nodes.mouse.y = -1000;
	  		Nodes.mouse.down = false;
	    });
    ...
    window.onresize = function(event) {
      ...
      Nodes.canvas.width = ($(canvasID).parents('.canvas-header').length > 0 ) ? window.innerWidth : $(canvasID).parents('.canvas-bg').width();
      Nodes.canvas.height = ($(canvasID).parents('.canvas-header').length > 0 ) ? window.innerHeight : $(canvasID).parents('.canvas-bg').height();
  ...
  draw: function() {
	  	...
	  	//if($('.no-scroll').length > 0) {
	  		...
	  		//$(Nodes.canvasID).parents('.canvas-header').find('.canvas-bg.topBoxOut').length == 0 && $(Nodes.canvasID).parents('.canvas-header').find('.canvas-bg.topBoxIn').length == 0
  ...
  // 		jQuery(Nodes.canvasID).parents('.canvas-bg').find('.canvas-bg').css({
  ...
  drawImageToBackground: function (index,index2,resize,sequence) {
    ...
    var $selector = '.canvas-header';
    var heightDiff = $('#full_screen.canvas-header .page').height()/1.3 + 50;
    ...
    this.bgContext.drawImage( shapeIndex, (this.canvas.width - newWidth) / 2, (((this.canvas.height+headerHeight/2) - newHeight - heightDiff*1) / 2.8) , newWidth, newHeight);
